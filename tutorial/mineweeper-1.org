...
#+begin_py
#+INCLUDE: "../../linkspace/examples/mineweeper.py/mineweeper-hotseat.py" src python
#+end_py
...
You can try the code with ~python ./examples/mineweeper.py/mineweeper-hotseat.py~
#+end_verse
...
#+begin_py
#+INCLUDE: "../../linkspace/examples/mineweeper.py/mineweeper.py" src python
#+end_py
#+end_ctabs
...
#+begin_src bash 
  ...
  find ./ -mindepth 1 -maxdepth 1 -type d -exec lk --dir "{}" write --pkts ./setup.pkt \;
#+end_src
...
#+begin_py
#+INCLUDE: "../../linkspace/examples/mineweeper.py/mineweeper-multiplayer.py" src python
#+end_py
...
Scrabble or Chess players on the internet can use tools you can't detect.
...
#+begin_src python
  ...
  ack_pkt = lk_keypoint( stamp=player_turn_pkt.stamp, link = [ack_link])~
  acked = dict()
  def collect_acks(ack_pkt):
      if list(ack_pkt.link) == [ack_link] and ack_pkt.stamp == ack_pkt.stamp:
