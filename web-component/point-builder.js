...
class PointBuilder extends HTMLElement {
  ...
  set space(v) {
    this.setAttribute("space", v || "");
  }
  get space() {
    return this.getAttribute("space");
  }
  set links(v) {
    return this.setAttribute("links", v || "");
  }
  get links() {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    const map = {
      space: () => {
        this.$("[name='space']").value = newValue;
      ...
      keypoint: () =>
        (this.$("button[value='keypoint']").style.display = newValue),
      linkpoint: () =>
        (this.$("button[value='linkpoint']").style.display = newValue),
      datapoint: () =>
        (this.$("button[value='datapoint']").style.display = newValue),
    };
    map[name]();
  }
  ...
  emitPreviewSpace() {
    let spaceEl = this.$("[name='space']");
    try {
      let [domain, group, path] = lka_space_expr(spaceEl.value);
      spaceEl.setCustomValidity("");
      spaceEl.reportValidity();
      this.dispatchEvent(
        new CustomEvent("preview-space", {
          composed: true,
          bubbles: true,
...
<span id="inputs">
 ...
 <input type="text" name="space" placeholder="domain:group:/path/name" />

<label for="data">Data</label>
<input type="text" name="data"/>

<label for="links">links</label>
<textarea name="links" placeholder="tag:ptr&#10;tag:ptr&#10;...">
</textarea>
</span>
...
`;
    ...
    this.$("[name='space']").addEventListener("change", () =>
      this.emitPreviewSpace(),
    );
  ...
  build_keypoint() {
    ...
    let [domain, group, path] = lka_space_expr(formdata.get("space"));
    let links = getLinks(formdata);
    return lk_keypoint(this.key, formdata.get("data"), {
      domain,
      group,
      path,
      links,
...
customElements.define("lkwc-point-builder", PointBuilder);
