...
Sometimes the Rust [[../docs/rust/linkspace/index.html][linkspace]] docs is ahead of this guide.
#+END_VERSE
...
The linkspace packet format is designed to be extremely fast in accessing its fields.
Depending on your use case, you can limit yourself to just those functions and emit packets as a dumb event streams over the network from scratch.
The library has various tools to make developing complex systems much easier.

The library is structured around 4 concepts.

- [[Point]] - A datagram containing application defined bytes with an optional spacename, links, and signature. 
- [[ABE]] - Language agnostic (byte) templating for convenience.
- [[Query]] - A list of predicates and options for selecting a set of points.
- [[#runtime][Runtime]] - A application driven runtime around a multi-reader single-writer database for saving and querying.

Using those tools some common challenges are addressed by a set of [[Conventions]].
...
- [[Point]] 
  ...
  - [[lk_keypoint]]
  - [[lk_serialize and lk_deserialize]]
...
#+BEGIN_VERSE
[[../docs/rust/linkspace/point/index.html][Rust docs]]
#+END_VERSE
...
Datapoints do not have a 'stamp' field, so they get the same hash given the same data.
If we had forced a specific stamp for both the python and bash example it would have produced the same hash for both.
By default 'stamp' is set to the current time ( microseconds since epoch ) and thus the hashes are different.
...
Where part[0..7] are the spacename components, 
...
#+begin_src python :session lkpy :exports both :results output verbatim
  ...
  packet_bytes = lk_serialize(datap) + lk_serialize(linkp) + lk_serialize(keyp)
  print(len(packet_bytes),packet_bytes)

  # read the bytes as packets
  (p1, packet_bytes)= lk_deserialize(packet_bytes)
  (p2, packet_bytes)= lk_deserialize(packet_bytes)
  (p3, packet_bytes)= lk_deserialize(packet_bytes)
...
#+BEGIN_VERSE
[[../docs/rust/linkspace/abe/index.html][Rust docs]]
#+END_VERSE
...
#+begin_src python :session lkpy :exports both :results output verbatim
  ...
  print(lk_encode(lkp.part0),"\t",list(lkp.part0))
  print(lk_encode(lkp.part1),"\t",list(lkp.part1))
  print(lk_encode(lkp.part2),"\t",list(lkp.part2))
  print(lk_encode(lkp.part3),"\t",list(lkp.part3))
  print(lk_encode(lkp.part4),"\t",list(lkp.part4), lkp.part4.decode("utf-8"))
#+end_src
...
#+BEGIN_VERSE
[[../docs/rust/linkspace/query/index.html][Rust docs]]
#+END_VERSE

A query is a list of predicates and options used to define a set of packets.
They're used in various ways, most notably you can use them to read ([[lk_read_one]], [[lk_read]]), await ([[lk_watch]]) and request ([[lk_pull]]) packets.
...
A predicate can be set multiple times. In the example above we could add ~stamp:<:[now:+2D]~ to constrain it further.
...
Here we look for the domain 'my' , the group [#:pub], with spacename starting at /hello and with one additional spacename part.
...
#+begin_py
Queries are mutable. They're created or copied with lk_query()
#+begin_src python :session lkpy :exports both :results value verbatim
  first = lk_query()
  lk_query_parse(first,"group:=:[#:pub]")
  second = lk_query(first)
  lk_query_print(second)
#+end_src
...
Add newline delimited constraints to a query.
...
#+begin_src python :session lkpy :exports both :results value verbatim
  try:
    lk_query_push(q,"data_size",">",bytes([0,100])) # greater than 100 and smaller than 3 can not both be true
  except Exception as e :
    return ("That's not possible",e)
...
Setting 'group', 'domain', 'spacename', 'links', or 'stamp' predicates automatically exclude the datapoint type.
...
#+Begin_VERSE
~group~ requires 32 bytes or attempts to parse base64.
~domain~ requires 16 bytes or **prepend** '\0' if too few bytes are given
...
They should use ~stamp~ to avoid depending on any group-exchange specific behavior.
...
Unlike predicates, options are not associative.
The order they are joined matters.
When using lk_query_push or lk_query_parse, the options are added *to the front* of the string.
i.e. finding the first matching option is done by reading from top to bottom.
Extensions should clearly define the meaning when multiple appear.

NOTE: These options will change somewhat in coming versions.

| name          | value                      | use                                            |
|---------------+----------------------------+------------------------------------------------|
| :read:        | (tree/hash/log)-(asc/desc) | set the index to read from                     |
| :follow       |                            | also output linked packets                     |
| :qid:         | <any>                      | query id                                       |
| :notify-close |                            | send a final dummy packet when closing a query |
...
Current set of [[../docs/rust/linkspace/query/enum.PredicateType.html][predicates]] and [[../docs/rust/linkspace/query/enum.KnownOptions.html][options]].
...
If you expect the value to be known locally [[lk_read_hashes]] is faster.
...
:END:
#+BEGIN_VERSE
[[../docs/rust/linkspace/index.html][Rust docs]]
#+END_VERSE

You can open/create a linkspace runtime with [[lk_open]].
It uses $LK_DIR or $HOME/linkspace by default.

A runtime is a handle to a multi-reader, single-writer database.
The instance is thread local. Each thread or process requires calling [[lk_open]].

An application can write to the database [[lk_write]].
To read from the database with a query use [[lk_read_one]], [[lk_read]].

You can register a query + callback with [[lk_watch]].

This callback is called for each newly written packet matching the query is written to the database.
Regardless which process or thread did the writing.

[[lk_tap]] combines lk_read and lk_watch.

The runtime is user driven.
A newly written packet isn't acknowledged by the runtime instance until a call to process is made: [[lk_process]],or [[lk_process_while]].
i.e.  ~lk_write; lk_process; lk_read;~ if you want to read back a packet you have just written.
...
:END:
#+BEGIN_VERSE
[[../docs/rust/linkspace/conventions/index.html][Rust docs]]
#+END_VERSE

Conventions are functions built on top of the other linkspace functions.
They provide a standard way for unrelated processes to loosly interface with each other by encoding data into linkspace packets.

One general conventions is that domains and space names starting with \xff are for meta things such as status queries and packet exchange.
...
#+BEGIN_SRC bash :session cli0 :exports both :results output verbatim
  python -c 'print("-" * 200000000,end="")' \
      | lk data \
      | lk collect example:: --stamp [epoch] --collect-tag 'data' --chain-tag 'continue'\
      | lk write -f \
      | lk filter example:: \
...
#+BEGIN_SRC bash :session cli0 :exports both :results output verbatim
  ...
  diff -q /tmp/bytes /tmp/bytes2  && echo Ok
#+END_SRC
...
**** Why Big Endian?
The tree index is in the expected order when using the numbers as space parts.
