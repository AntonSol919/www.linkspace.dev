...
They're used in various ways, most notably you can use them to read ([[lk_read]], [[lk_read_all]]), await ([[lk_watch]]) and request ([[lk_pull]]) packets.
...
Current set of [[../docs/rust/linkspace/query/enum.PredicateType.html][predicates]] and 
[[../docs/rust/linkspace/query/enum.KnownOptions.html][options]].
...
To read from the database you can directly get a packet by hash ([[lk_read_hashes]]) or using a query ([[lk_read]], [[lk_read_all]]).
...
This includes calls to ~lk_read*~ and ~lk_watch~.  
...
The [[../docs/rust/linkspace/index.html][Rust docs]] are currently the most up to date, but the python package has most functions typed and commented as well.
